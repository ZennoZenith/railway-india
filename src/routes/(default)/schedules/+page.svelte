...
import type { ScheduleRow, TrainGeneralInfo } from "api-railway/dist/types";
import { tick } from "svelte";
import Schedule from "./Schedule.svelte";

let response = $state<
  Superposition<{ trainNumber: [string] }, ScheduleRow[]>
>();
...
async function onFormSubmit(
  ...
  if (!trainNumber || trainNumber.length === 0) {
    ...
    } as Superposition<{ trainNumber: [string] }, ScheduleRow[]>;
    return;
  }
